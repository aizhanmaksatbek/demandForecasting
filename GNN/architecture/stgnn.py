...
def causal_conv1d(x: torch.Tensor, conv: nn.Conv1d, dilation: int) -> torch.Tensor:
    """
...
class TemporalBlock(nn.Module):
    ...
    def __init__(self, c_in: int, c_out: int, kernel_size: int, dilation: int, dropout: float):
        ...
        self.downsample = nn.Conv1d(c_in, c_out, kernel_size=1) if c_in != c_out else nn.Identity()
        self.act = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y = self.dropout(y)
        # residual
        res = causal_conv1d(z, self.downsample, dilation=1) if not isinstance(self.downsample, nn.Identity) else z
        y = self.act(y + res)
        # back to [B, T, N, C_out]
        T_out = y.shape[-1]
...
class GraphConv(nn.Module):
    ...
    Output: [B, T, N, C_out]
    """
    def __init__(self, c_in: int, c_out: int, A_hat: torch.Tensor, bias: bool = True):
        super().__init__()
        self.A_hat = nn.Parameter(A_hat, requires_grad=False)  # fixed adjacency
...
class STBlock(nn.Module):
    ...
    """
    def __init__(self, c_in: int, c_hidden: int, c_out: int, A_hat: torch.Tensor, t_kernel: int, dilation: int, dropout: float):
        super().__init__()
        self.temp1 = TemporalBlock(c_in, c_hidden, kernel_size=t_kernel, dilation=dilation, dropout=dropout)
        self.gconv = GraphConv(c_hidden, c_hidden, A_hat=A_hat)
        self.temp2 = TemporalBlock(c_hidden, c_out, kernel_size=t_kernel, dilation=dilation, dropout=dropout)
        self.res_proj = nn.Linear(c_in, c_out) if c_in != c_out else nn.Identity()
        self.act = nn.ReLU()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if isinstance(self.res_proj, nn.Identity):
            ...
            r = self.res_proj(res.reshape(B * T * N, C_in)).reshape(B, T, N, -1)
            out = self.act(x + r)
        return out
...
class STGNN(nn.Module):
    ...
    def forward(self, x_hist: torch.Tensor) -> torch.Tensor:
        ...
        B, N, _ = out.shape
        out = out.view(B, N, self.horizon, len(self.quantiles)).permute(0, 2, 1, 3)  # [B, H, N, Q]
        return out
