...
def main():
    parser = argparse.ArgumentParser(description="STGNN explainer + evaluator")
    parser.add_argument("--ckpt", type=str, default=os.path.join("GNN", "checkpoints", "stgnn_best.pt"))
    parser.add_argument("--panel-csv", type=str, default=os.path.join("GNN", "data", "panel.csv"))
    parser.add_argument("--node-index-csv", type=str, default=os.path.join("GNN", "data", "node_index.csv"))
    parser.add_argument("--adj", type=str, default=os.path.join("GNN", "data", "adjacency.npy"))
    parser.add_argument("--target-node", type=int, default=0)
    parser.add_argument("--batch-index", type=int, default=0)
    parser.add_argument("--out-dir", type=str, default=os.path.join("GNN", "checkpoints"))
    parser.add_argument("--algo", type=str, choices=["gnn", "custom"], default="custom")
    parser.add_argument("--evaluate", action="store_true")
    parser.add_argument("--batch-size", type=int, default=None)
    parser.add_argument("--top-k", type=int, default=20, help="Top-K neighbors to export/plot for custom explainer")
    parser.add_argument("--no-plot", action="store_true", help="Skip generating the bar plot for neighbor importances")
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    assert os.path.exists(args.ckpt), f"Checkpoint not found: {args.ckpt}"
    assert os.path.exists(args.panel_csv), f"panel.csv not found: {args.panel_csv}"
    assert os.path.exists(args.node_index_csv), f"node_index.csv not found: {args.node_index_csv}"
    ...
    if len(test_ds) == 0:
        raise RuntimeError("Test dataset is empty. Check your data and split bounds.")
    ...
    if args.evaluate:
        ...
        with torch.no_grad():
            for batch in test_loader:
                ...
                preds.append(yhat[..., med].cpu().numpy())
        pinball = total_loss / max(len(test_loader.dataset), 1)
        y_flat = np.concatenate(ys, axis=0).reshape(-1)
        p_flat = np.concatenate(preds, axis=0).reshape(-1)
        print({"split": "test", "pinball": round(pinball, 6),
               "wape": round(wape(y_flat, p_flat), 6),
               "smape": round(smape(y_flat, p_flat), 6)})
