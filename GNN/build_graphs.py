...
def build_store_graph(stores: pd.DataFrame, trans: pd.DataFrame, corr_min=0.2):
    ...
    for i, a in enumerate(snbrs):
        for b in snbrs[i + 1:]:
            ...
            if (
                pd.notna(ra["cluster"])
                and pd.notna(rb["cluster"])
                and ra["cluster"] == rb["cluster"]
            ):
                w += 0.5
            if (
                pd.notna(ra["state"])
                and pd.notna(rb["state"])
                and ra["state"] == rb["state"]
            ):
                w += 0.3
    ...
    return pd.DataFrame(
        edges,
        columns=["store_nbr_src", "store_nbr_dst", "weight"]
        )


def build_family_graph(train: pd.DataFrame, corr_min=0.3):
    fam_daily = (train.groupby(["date", "family"])["sales"]
                 .sum()
                 .unstack("family")
                 .fillna(0.0)
                 )
...
if __name__ == "__main__":
    ...
    trans = pd.read_csv(os.path.join(RAW, "transactions.csv"),
                        parse_dates=["date"]
                        )

    # Build edge lists
    store_edges = build_store_graph(stores, trans)
    fam_edges = build_family_graph(train)

    store_edges.to_csv(
        os.path.join(PROC, "store_graph_edges.csv"), index=False
        )
    fam_edges.to_csv(
        os.path.join(PROC, "family_graph_edges.csv"), index=False
        )
    ...
    A_s = edges_to_adj(stores_u, store_edges,
                       "store_nbr_src", "store_nbr_dst", "weight",
                       undirected=True
                       )
    A_f = edges_to_adj(families_u, fam_edges,
                       "family_src", "family_dst", "weight",
                       undirected=True
                       )
    ...
    print(f"Saved adjacency.npy with shape {A_kron.shape}\
          and node_index.csv with {len(node_index)} nodes.")
